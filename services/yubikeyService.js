...
async function getRegistrationOptions() {
  ...
  opts.rp.name = "API Setup Service";
...
async function verifyRegistration(attestationResponse) {
  ...
  const result = await fido2.attestationResult(attestationResponse, {
    ...
    origin: resolveExpectedOrigin(process.env.WEBAUTHN_ORIGINS || appCypherConfig.WEBAUTHN_ORIGIN || "http://localhost:3201"),
    factor: "either",
    rpId: appCypherConfig.RP_ID || "localhost",
  });
  ...
  console.log(`[verifyRegistration] Registered credential ID: ${globalCredential.credId}`);
  console.log(`[verifyRegistration] Registered credential counter: ${globalCredential.counter}`);
  console.log(`[verifyRegistration] Registered credential public key: ${globalCredential.publicKey}`);
...
async function getAuthenticationOptions() {
  ...
  console.log(`[getAuthenticationOptions] CredId: ${globalCredential.credId}`);
...
// encrypt data to send to other server with public key and decrypt with private key
